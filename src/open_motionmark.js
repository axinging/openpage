...
async function openPage(url, browserArgs) {
  ...
  await page.goto(url);
  // Click run benchmark
  const runButton = page.locator('button:has-text("Run Benchmark")').first();
  await runButton.click();
  // console.log(global.result);

  //const id = '#results-score > tbody > tr:nth-child(10) > td';
...
async function runSingleBenchmark(repeat, draw = 500, tag = "") {
  ...
  var average = 0;
  var averages = [];
  var url = `https://browserbench.org/MotionMark/`;
