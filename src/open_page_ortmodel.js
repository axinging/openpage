...
// "C:\Users\abc\AppData\Local\Google\Chrome SxS\Application\chrome.exe" --enable-dawn-features=allow_unsafe_apis,use_dxc --enable-features=SharedArrayBuffer  --dump-shaders
  ...
  */
  ...
  page.on('console', async msg => {
    ...
    if (msgStr.includes('ortend')) {
        console.log(msgStr);
        global.results['pass'][model] = 'pass';
        exitCondition.logEnd = true;
    }
  });
  page.on('pageerror', (err) => {
    console.log("xxx" + err.message);
    global.results['fail'][model] = err.message;
    //console.log(msgStr);
    exitCondition.logEnd = true;
  });
  return [context, page];
